...
const footerSections = [
  ...
  },
  {
    title: 'RECURSOS',
    links: [
      { label: 'Biblioteca COMEX', to: '#' },
      { label: 'Guías y Normativas', to: '#' },
      { label: 'Noticias', to: '#' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <div className="flex gap-3">
              <a href="#" aria-label="LinkedIn" className="text-white/40 hover:text-gold transition-colors">
          ...
          {footerSections.map((section) => (
            <div key={section.title}>
              <h4 className="text-white/50 text-xs font-semibold tracking-widest mb-4">{section.title}</h4>
              <ul className="space-y-2">
                {section.links.map((link) => (
                  <li key={link.label}>
                    <Link to={link.to} className="text-white/40 hover:text-white/70 text-sm transition-colors">
                      {link.label}
                    </Link>
                  </li>
                ))}
        ...
        <div className="mt-10 pt-6 border-t border-white/5 text-center">
          <p className="text-white/20 text-xs">
            © {new Date().getFullYear()} FOCUS COMEX. Todos los derechos reservados.
          </p>
        </div>
      </div>
    </footer>
